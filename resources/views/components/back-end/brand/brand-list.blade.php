...
<script>
    ...
    async function getList() {
        try {
            // Show loader when fetching the list
            showLoader();
        ...
        hideLoader();
            ...
            // Loop through the brand data and append rows
            res.data['BrandData'].forEach(function(item, index) {
                let row = `
                   <tr data-row="${index + 1}" data-date="2024-08-05">
                       <td>${index + 1}</td>
                       <td><img style="width: 60px; height: auto" alt="" src="${item['logo']}"></td>
                       <td>${item['name']}</td>
                       <td>${item['status']}</td>
                       <td>
                           <div id="action_btn">
                               <div id="menu-wrap">
                                   ...
                                   <div class="menu">
                                       <div>
                                           <ul>
                                               <li><a href="#" class="link custom-open-modal-btn" data-id="${item['id']}" data-modal="custom-modal-1">Edit</a></li>
                                               <li><a href="#" class="link custom-open-delet-modal-btn" data-modal="custom-modal-2" data-id="${item['id']}">Delete</a></li>
                                           </ul>
                                       </div>
                                   </div>
                               </div>
            ...
            $(document).on('click', '.custom-open-modal-btn', async function() {
                let id = $(this).data('id');
                await FillUpUpdateForm(id); // Ensure async logic works here
                $("#custom-modal-1").modal('show'); // Open modal
            });

            $(document).on('click', '.custom-open-delet-modal-btn', function() {
                let id = $(this).data('id');
                $("#custom-modal-2").modal('show'); // Open delete modal
